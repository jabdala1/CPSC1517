...
namespace NorthwindSystem.BLL
{
    ...
    public class ProductController
    {
        ...
        //update will recieve an instance <T> that contains the needed primary key values
        //the method will return the number of rows affected
        public int Product_Update(Product item)
        {
            using (var context = new NorthwindContext())
            {
                //sometimes you may have additional fields on your entity that track dates and times that the record was altered
                //these field should be set by the controller method and NOT be alter/set by the application user
                //assume that our entity has a LastModified date
                //item.LastModified = DateTime.Today;
                //stage
                context.Entry(item).State = System.Data.Entity.EntityState.Modified;
                //commit
                //the value return from the SaveChanges() is the number of rows affected by the update
        ...
        public int Product_Delete(int productid)
        {
            ...
            {
                //physical delete
                //removal of the record form the database
                ...
                //Find the record on the database
                var existing = context.Products.Find(productid);
                //alter the appropriate field(s) on the record which will apply the logical delete
